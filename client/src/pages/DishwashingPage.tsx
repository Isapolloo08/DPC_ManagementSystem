...
import { 
  ...
  Check, Edit, X, Church, BookOpen, UserCheck, Handshake, Users2
} from "lucide-react";
...
export const DishwashingPage: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 sm:p-6 rounded-3xl border border-indigo-100/80 shadow-2xs">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="p-2 rounded-xl bg-gradient-to-br from-amber-400 to-amber-600 text-white shadow-xs">
              <Utensils className="w-5 h-5" />
            </span>
            <h1 className="text-xl sm:text-2xl font-black text-charcoal tracking-tight">
              Dishwashing & Kitchen Roster
            </h1>
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-indigo-50 text-indigo border border-indigo-100 flex items-center gap-1">
              <Handshake className="w-3 h-3 text-amber-500" />
              <span>Multi-Team Rotation</span>
            </span>
          </div>
          <p className="text-xs sm:text-sm text-charcoal/60">
            Schedule fellowship meal dishwashing duties by single group or <strong>team up multiple Bible Study Groups & Ministries</strong> with automated leader autofills.
          </p>
        </div>
        ...
        {isAdminOrCoordinator && (
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2.5 rounded-2xl bg-indigo hover:bg-indigo-700 text-white text-xs font-bold shadow-md hover:shadow-lg transition-all active:scale-95 cursor-pointer"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-2.5 rounded-2xl bg-ivory-light hover:bg-gray-100 text-charcoal text-xs font-bold border border-gray-200 transition-all cursor-pointer"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
        ...
        <div className="lg:col-span-7 bg-gradient-to-br from-indigo-950 via-indigo-900 to-indigo-950 rounded-3xl p-6 sm:p-7 text-white relative overflow-hidden shadow-xl border border-white/10 flex flex-col justify-between">
          ...
          <div className="relative z-10 space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-400/20 border border-amber-400/40 text-amber-300 text-xs font-black tracking-wider uppercase shadow-xs">
              ...
              {thisSunday && (
                <div className="flex items-center gap-1.5">
                  {thisSunday.partner_assigned_name && (
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-amber-400 text-indigo-950 flex items-center gap-1">
                      <Handshake className="w-3 h-3" />
                      <span>Joint Duty (Teamed Up)</span>
                    </span>
                  )}
                  <span className={`px-3 py-0.5 rounded-full text-xs font-extrabold uppercase ${
                    ...
                    {thisSunday.status === "completed" ? "✓ Done" : "⏳ Active On Duty"}
                  </span>
                </div>
              )}
            </div>
            ...
            {thisSunday ? (
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="px-2.5 py-0.5 rounded-lg text-xs font-bold bg-white/15 text-indigo-100 border border-white/10">
                    {thisSunday.cycle_mode === "biblestudy_group" ? "Bible Study Group" : "Ministry Department"}
                  </span>
                  {thisSunday.partner_assigned_name && (
                    <span className="px-2.5 py-0.5 rounded-lg text-xs font-bold bg-amber-400/20 text-amber-200 border border-amber-400/30 flex items-center gap-1">
                      <Users2 className="w-3 h-3" />
                      <span>Teamed Up Teams</span>
                    </span>
                  )}
                </div>
              ...
              <div className="py-4 text-center text-indigo-200 text-sm">
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs text-indigo-100">
                ...
                {thisSunday.leader_name && (
                  <div className="flex items-center gap-1.5 font-medium">
                    ...
                    <span>In-Charge: <strong>{thisSunday.leader_name}</strong> {thisSunday.partner_leader_name ? `& ${thisSunday.partner_leader_name}` : ''}</span>
                  </div>
                )}
              </div>
              ...
              {isAdminOrCoordinator && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-2 rounded-xl text-xs font-bold bg-white/10 hover:bg-white/20 text-white transition-all cursor-pointer flex items-center gap-1"
                  >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-1.5 cursor-pointer ${
                      thisSunday.status === "completed"
                        ? "bg-white/20 text-white hover:bg-white/30"
                        : "bg-amber-400 hover:bg-amber-300 text-indigo-950 font-black"
                    }`}
                  >
        ...
        <div className="lg:col-span-5 bg-white rounded-3xl p-6 border border-indigo-100/80 shadow-2xs flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-800 text-xs font-bold uppercase tracking-wider">
                <Clock className="w-3.5 h-3.5 text-indigo-600" />
                <span>NEXT SUNDAY IN ROTATION</span>
              </div>
              <span className="text-[10px] font-bold text-charcoal/50">Next Turn</span>
            </div>
            ...
            {nextSunday ? (
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] px-2 py-0.5 rounded bg-gray-100 text-charcoal/70 font-semibold">
                    {nextSunday.cycle_mode === "biblestudy_group" ? "Bible Study Group" : "Ministry"}
                  </span>
                  {nextSunday.partner_assigned_name && (
                    <span className="text-[10px] px-2 py-0.5 rounded bg-amber-100 text-amber-900 font-bold flex items-center gap-1">
                      <Handshake className="w-3 h-3" />
                ...
                <div className="bg-ivory-light p-3.5 rounded-2xl border border-gray-100 space-y-1.5 text-xs">
                  ...
                  {nextSunday.leader_name && (
                    <div className="flex items-center justify-between text-charcoal/80 font-medium">
                      <span>In-Charge / Leaders:</span>
                      <strong>{nextSunday.leader_name} {nextSunday.partner_leader_name ? `& ${nextSunday.partner_leader_name}` : ''}</strong>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between text-[11px] text-charcoal/60">
            <span>Automated loop rotation active</span>
            {isAdminOrCoordinator && nextSunday && (
              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleOpenEditModal(nextSunday)}
                  className="text-xs font-bold text-charcoal/70 hover:text-indigo flex items-center gap-1 cursor-pointer"
                >
                  <Edit className="w-3.5 h-3.5" />
                  <span>Edit</span>
                </button>
                <button
                  ...
                  className="text-xs font-bold text-indigo hover:text-indigo-700 flex items-center gap-1 cursor-pointer"
                >
      ...
      <div className="bg-white p-5 sm:p-6 rounded-3xl border border-indigo-100/80 shadow-2xs space-y-3">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2">
            <RotateCw className="w-4 h-4 text-indigo" />
          ...
          <span className="text-xs font-semibold text-indigo bg-indigo-50 px-2.5 py-0.5 rounded-full">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 pt-1 no-scrollbar">
          {duties.slice(0, 8).map((item, idx) => {
            ...
            return (
              <React.Fragment key={item.id}>
                <div className={`shrink-0 p-3 rounded-2xl border transition-all text-xs space-y-1 min-w-[180px] ${
                  isThis 
                    ? "bg-indigo-900 text-white border-indigo-700 shadow-md scale-102" 
                    : isNext
                  ...
                  <div className="flex items-center justify-between text-[10px] font-bold">
                    <span className={isThis ? "text-amber-300" : "text-charcoal/50"}>
                      Turn #{item.cycle_order_index || idx + 1}
                    </span>
                    {isThis && <span className="bg-amber-400 text-indigo-950 px-1.5 rounded-full font-black">THIS SUN</span>}
                    {isNext && <span className="bg-indigo text-white px-1.5 rounded-full">NEXT</span>}
                    {isDone && <span className="text-emerald-600 font-black">✓ DONE</span>}
                  </div>
                  <h4 className={`font-bold truncate ${isThis ? "text-white" : "text-charcoal"}`}>
                    {item.assigned_name}
                    {item.partner_assigned_name && ` & ${item.partner_assigned_name}`}
                  </h4>
                  <p className={`text-[10px] truncate ${isThis ? "text-indigo-200" : "text-charcoal/60"}`}>
          ...
          {duties.length > 8 && (
            <div className="shrink-0 text-xs font-bold text-charcoal/50 px-3 py-2 bg-gray-100 rounded-xl">
      ...
      <div className="bg-white p-4 rounded-2xl border border-indigo-100/80 shadow-2xs flex flex-col md:flex-row items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
          ...
          <div className="flex items-center gap-1.5 text-xs font-bold text-charcoal/70">
            ...
            <select
              ...
              className="bg-ivory-light px-3 py-1.5 rounded-xl border border-gray-200 focus:outline-none focus:border-indigo font-bold text-indigo cursor-pointer text-xs"
            >
        ...
        <div className="w-full md:w-72">
          <input
            ...
            className="w-full bg-ivory-light px-3.5 py-1.5 rounded-xl text-xs border border-gray-200 focus:outline-none focus:border-indigo"
          />
        </div>
      </div>

      {/* DUTIES SCHEDULE TABLE */}
      <div className="bg-white rounded-3xl border border-indigo-100/80 shadow-2xs overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-charcoal/40 text-xs space-y-2">
            <p>No dishwashing duties scheduled yet.</p>
            {isAdminOrCoordinator && (
              <button
                onClick={handleOpenAddModal}
                className="px-4 py-2 bg-indigo text-white text-xs font-bold rounded-xl shadow cursor-pointer inline-flex items-center gap-1.5"
              >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-ivory-light/80 text-charcoal/70 font-bold border-b border-gray-100 text-[11px] uppercase tracking-wider">
                  <th className="py-3 px-4">Sunday Date</th>
                  <th className="py-3 px-4">Assigned Entity (Teamed Up Groups/Ministries)</th>
                  <th className="py-3 px-4">Assignment Type</th>
                  <th className="py-3 px-4">In-Charge / Leaders</th>
              ...
              <tbody className="divide-y divide-gray-100 font-medium">
                {filteredDuties.map((d) => {
                  ...
                  return (
                    <tr 
                      key={d.id} 
                      className={`hover:bg-ivory-light/50 transition-colors ${
                        isThis ? "bg-amber-50/40 font-semibold" : ""
                      }`}
                    >
                      <td className="py-3.5 px-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          {isThis && (
                            <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-amber-400 text-indigo-950">
                              THIS SUN
                            </span>
                          )}
                        </div>
                      ...
                      <td className="py-3.5 px-4">
                        <div className="space-y-0.5">
                          <div className="font-bold text-charcoal flex items-center gap-2 flex-wrap">
                          ...
                          {isJoint && (
                            <span className="inline-flex items-center gap-1 text-[9px] font-black px-2 py-0.2 rounded-full bg-amber-50 text-amber-800 border border-amber-200">
                      ...
                      <td className="py-3.5 px-4 whitespace-nowrap">
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-gray-100 text-charcoal/70 uppercase">
                          ...
                          {isAdminOrCoordinator && (
                            <>
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-lg bg-ivory-light text-indigo hover:bg-indigo-50 border border-gray-200 transition-all cursor-pointer"
                              >
                                <Edit className="w-3.5 h-3.5" />
                              </button>
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-lg bg-ivory-light text-amber-600 hover:bg-amber-50 border border-gray-200 transition-all cursor-pointer"
                              >
                                <ArrowLeftRight className="w-3.5 h-3.5" />
                              </button>
                              ...
                              <button
                                ...
                                title="Delete duty"
                                className="p-1.5 rounded-lg bg-ivory-light text-rose hover:bg-rose-50 border border-gray-200 transition-all cursor-pointer"
                              >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        >
                          <Church className={`w-3.5 h-3.5 shrink-0 ${team.cycle_mode === "ministry" ? "text-amber-300" : "text-indigo"}`} />
