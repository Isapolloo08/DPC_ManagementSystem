    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-charcoal flex items-center gap-2">
            <span>Giving & Stewardship</span>
          </h1>
          <p className="text-xs text-charcoal/60 mt-0.5">
            Ministry fund progress, tithes & offerings, and official tax contribution statements.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-amber hover:bg-amber-400 text-charcoal font-bold px-4 py-2 rounded-xl text-xs shadow-sm transition-all"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {funds.map((fund) => {
          ...
          return (
            <div key={fund.id} className="bg-white rounded-2xl p-5 border border-indigo-100/80 shadow-2xs flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="p-2 rounded-xl bg-amber-50 text-amber-600">
                    <Heart className="w-4 h-4" />
                  </span>
                  <span className="text-xs font-bold text-sage-800 bg-sage-50 px-2 py-0.5 rounded-full">
                    {pct}% Funded
                  </span>
                </div>

                <h3 className="font-bold text-sm text-charcoal">{fund.name}</h3>
              ...
              <div className="mt-4 pt-3 border-t border-gray-100">
                <div className="flex justify-between text-xs font-bold mb-1.5">
                  <span className="text-indigo">${raised.toLocaleString()}</span>
                  <span className="text-charcoal/50">Goal: ${target.toLocaleString()}</span>
                </div>
                <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
                  <div
                    className="bg-gradient-to-r from-amber to-amber-500 h-full rounded-full transition-all duration-500"
                    style={{ width: `${pct}%` }}
                  ></div>
                </div>
              </div>
      ...
      <div className="bg-white rounded-2xl border border-indigo-100/80 shadow-2xs overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-sm font-bold text-charcoal flex items-center gap-2">
            <DollarSign className="w-4 h-4 text-indigo" />
            <span>Contribution History</span>
          </h2>
          <span className="text-xs text-charcoal/50">{donations.length} recorded gifts</span>
        </div>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-ivory text-charcoal/70 uppercase text-[10px] font-bold tracking-wider">
              <tr>
                ...
                <th className="p-3.5">Fund Designation</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {donations.map((d) => (
                <tr key={d.id} className="hover:bg-ivory-light/60">
                  <td className="p-3.5 font-bold text-indigo">
                    {d.first_name ? `${d.first_name} ${d.last_name}` : "Anonymous / Direct"}
                  </td>
                  <td className="p-3.5 font-medium text-charcoal">{d.fund_name}</td>
                  <td className="p-3.5 font-bold text-charcoal">${d.amount.toFixed(2)}</td>
                  <td className="p-3.5">
                    <span className="px-2 py-0.5 rounded text-[10px] font-semibold uppercase bg-indigo-50 text-indigo-800">
                      {d.method}
                    </span>
                  </td>
                  <td className="p-3.5 text-charcoal/60">{d.notes || "—"}</td>
                  <td className="p-3.5 text-right text-charcoal/50">
        ...
        <div className="fixed inset-0 z-50 bg-charcoal/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-gray-100">
              ...
              <button onClick={() => setIsGiveModalOpen(false)} className="p-1 text-charcoal/50">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleRecordDonation} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-charcoal/70 mb-1">Fund Designation *</label>
                <select
              ...
              <div>
                <label className="block font-bold text-charcoal/70 mb-1">Gift Amount ($ USD) *</label>
                <input
                  type="number"
                  step="0.01"
                  min="1"
                  ...
                  className="w-full bg-ivory-light font-bold text-base p-2 rounded-xl border border-gray-200 focus:outline-none focus:border-indigo"
                />
              </div>

              <div>
                <label className="block font-bold text-charcoal/70 mb-1">Payment Method</label>
                <select
                  value={giveForm.method}
                  onChange={(e) => setGiveForm({ ...giveForm, method: e.target.value })}
                  className="w-full bg-ivory-light p-2 rounded-xl border border-gray-200 focus:outline-none focus:border-indigo font-medium"
                >
              ...
              <div>
                <label className="block font-bold text-charcoal/70 mb-1">Notes / Dedication</label>
                <input
                  type="text"
              ...
              <div className="pt-3 border-t border-gray-100 flex items-center justify-end gap-2">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl bg-amber hover:bg-amber-400 text-charcoal font-bold shadow-md"
                >
        ...
        <div className="fixed inset-0 z-50 bg-charcoal/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full p-8 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-gray-200">
              <div>
                <div className="text-xs font-bold uppercase tracking-wider text-amber-700">OFFICIAL ANNUAL CONTRIBUTION STATEMENT</div>
                <h2 className="text-xl font-serif font-bold text-charcoal mt-0.5">{statementData.organization}</h2>
                <p className="text-xs text-charcoal/50">Tax ID / 501(c)(3): {statementData.tax_id}</p>
              </div>
              <button onClick={() => setStatementData(null)} className="p-1 text-charcoal/50 hover:text-charcoal">
                <X className="w-5 h-5" />
              </button>
            </div>

            {/* Member & Tax Year */}
            <div className="grid grid-cols-2 gap-4 text-xs bg-ivory p-4 rounded-xl border border-amber/20">
              <div>
                <span className="text-charcoal/50 block font-bold">DONOR:</span>
                <span className="font-bold text-base text-indigo">{statementData.member.name}</span>
              ...
              <div className="text-right">
                ...
                <span className="font-mono font-bold text-base text-charcoal">{statementData.statement_year}</span>
                <p className="text-charcoal/50 mt-1">Issued: {new Date(statementData.issued_date).toLocaleDateString()}</p>
              </div>
            </div>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tfoot className="border-t-2 border-charcoal">
                <tr>
                  ...
                  <td className="p-3 text-right font-black text-base text-indigo">${statementData.total_giving.toFixed(2)}</td>
                </tr>
              </tfoot>
            </table>
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              <button
                ...
                className="px-4 py-2 rounded-xl bg-gray-100 font-semibold text-xs text-charcoal"
              >
                Close
              </button>
              <button
                onClick={() => window.print()}
                className="flex items-center gap-1.5 px-5 py-2 rounded-xl bg-indigo hover:bg-indigo-700 text-white font-bold text-xs shadow-md"
              >
