...
function generateSecurityCode(ministryName) {
...
router.get("/today", async (req, res) => {
    ...
    `;
        ...
        const rows = await schema_1.db.all(query, params);
        res.json(rows);
    }
    catch (err) {
...
router.post("/check-in", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin", "Coordinator", "Volunteer"), async (req, res) => {
    try {
        const { member_id, ministry_id, event_id, notes, service_name } = req.body;
        if (!member_id) {
            return res.status(400).json({ error: "member_id is required" });
        }
        const member = await schema_1.db.get("SELECT * FROM members WHERE id = $1", [member_id]);
        if (!member) {
            return res.status(404).json({ error: "Member not found" });
        }
        const assignedMinistryId = ministry_id || member.ministry_id || 1;
        const ministry = await schema_1.db.get("SELECT * FROM ministries WHERE id = $1", [assignedMinistryId]);
        if (!ministry) {
    ...
    `, [
        ...
        await (0, auth_1.logAuditAction)(req.user?.id || null, "CHECK_IN", "attendance", newId, `Sunday Worship Attendance: ${member.first_name} ${member.last_name} (${ministry.name})`);
        res.status(201).json({
            id: newId,
...
router.post("/batch-check-in", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin", "Coordinator", "Volunteer"), async (req, res) => {
    try {
        ...
        for (const mId of member_ids) {
            const member = await schema_1.db.get("SELECT * FROM members WHERE id = $1", [mId]);
            if (!member)
                continue;
            const ministryId = member.ministry_id || 1;
            const ministry = await schema_1.db.get("SELECT * FROM ministries WHERE id = $1", [ministryId]);
            // Check if already checked in today
            const existing = await schema_1.db.get("SELECT id FROM attendance WHERE member_id = $1 AND DATE(checked_in_at) = CURRENT_DATE", [mId]);
            if (existing)
                continue;
            let securityCode = null;
      ...
      `, [mId, ministryId, securityCode, req.user?.id || null, service_name || "Sunday Worship Household Check-in"]);
        ...
        await (0, auth_1.logAuditAction)(req.user?.id || null, "CHECK_IN", "attendance", null, `Batch check-in performed for ${checkedInMembers.length} members`);
        res.status(201).json({
...
router.post("/check-out", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin", "Coordinator", "Volunteer"), async (req, res) => {
    try {
        const { attendance_id, member_id, security_code } = req.body;
        let record = null;
        if (attendance_id) {
            record = await schema_1.db.get(`
      ...
      WHERE id = $2
    `, [now, record.id]);
        await (0, auth_1.logAuditAction)(req.user?.id || null, "CHECK_OUT", "attendance", record.id, `Checked out attendance #${record.id}`);
        res.json({
...
router.delete("/:id", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin", "Coordinator", "Volunteer"), async (req, res) => {
    try {
        const { id } = req.params;
        const current = await schema_1.db.get("SELECT a.*, m.first_name, m.last_name FROM attendance a JOIN members m ON a.member_id = m.id WHERE a.id = $1", [id]);
        if (!current)
            return res.status(404).json({ error: "Attendance record not found" });
        await schema_1.db.run("DELETE FROM attendance WHERE id = $1", [id]);
        await (0, auth_1.logAuditAction)(req.user?.id || null, "DELETE", "attendance", Number(id), `Undid attendance mark for ${current.first_name} ${current.last_name}`);
...
router.get("/trends", async (req, res) => {
    try {
        ...
        const ministries = await schema_1.db.all(query, params);
        const trends = await Promise.all(ministries.map(async (m) => {
            const totalCheckins = await schema_1.db.get(`
        SELECT COUNT(*) as count FROM attendance WHERE ministry_id = $1
      `, [m.id]);
            const weeklyBreakdown = await schema_1.db.all(`
