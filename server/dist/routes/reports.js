...
router.get("/dashboard", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const totalMembers = await schema_1.db.get(totalMembersQuery, queryParams);
        const totalHouseholds = await schema_1.db.get("SELECT COUNT(*) as count FROM households");
        const todayCheckins = await schema_1.db.get(todayCheckinsQuery, queryParams);
        const totalDonationsYTD = await schema_1.db.get("SELECT COALESCE(SUM(amount), 0) as total FROM donations WHERE EXTRACT(YEAR FROM donated_at) = EXTRACT(YEAR FROM CURRENT_DATE)");
        const openPrayers = await schema_1.db.get("SELECT COUNT(*) as count FROM prayer_requests WHERE status = 'open'");
        const upcomingEvents = await schema_1.db.get(upcomingEventsQuery, queryParams);
        ...
        const ministries = await schema_1.db.all(minQuery, minParams);
        const ministryBreakdown = await Promise.all(ministries.map(async (m) => {
            const count = await schema_1.db.get("SELECT COUNT(*) as count FROM members WHERE ministry_id = $1 AND status = 'active'", [m.id]);
            const checkins = await schema_1.db.get("SELECT COUNT(*) as count FROM attendance WHERE ministry_id = $1 AND DATE(checked_in_at) = CURRENT_DATE", [m.id]);
            return {
                id: m.id,
                name: m.name,
                color: m.color,
    ...
    `;
        ...
        const members = await schema_1.db.all(agingOutQuery, agingParams);
        const agingOutAlerts = members.filter(m => {
            ...
            const age = (0, ministries_1.calculateAge)(bStr);
