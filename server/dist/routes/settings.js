...
router.get("/lookups", async (req, res) => {
    try {
        ...
        let list = await schema_1.db.all(query, params);
        ...
        `, [item.type, item.name, item.description, item.color, item.sort_order]);
            ...
            list = await schema_1.db.all(query, params);
        }
        res.json(list || []);
    }
    catch (err) {
...
router.get("/lookups/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const lookup = await schema_1.db.get("SELECT * FROM system_lookups WHERE id = $1", [id]);
        if (!lookup)
...
router.post("/lookups", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin"), async (req, res) => {
    try {
        ...
        const existing = await schema_1.db.get("SELECT id FROM system_lookups WHERE type = $1 AND LOWER(name) = LOWER($2)", [type.trim(), name.trim()]);
        if (existing) {
    ...
    `, [
        ...
        await (0, auth_1.logAuditAction)(req.user?.id || null, "CREATE", "system_lookups", newId, `Created ${type}: ${name.trim()}`);
...
router.put("/lookups/:id", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin"), async (req, res) => {
    try {
        const id = req.params.id;
        const { name, description, color, sort_order, is_active } = req.body;
        const current = await schema_1.db.get("SELECT * FROM system_lookups WHERE id = $1", [id]);
        if (!current)
            return res.status(404).json({ error: "Lookup not found" });
        const updatedName = name !== undefined ? name.trim() : current.name;
        if (name && name.trim().toLowerCase() !== current.name.toLowerCase()) {
            const duplicate = await schema_1.db.get("SELECT id FROM system_lookups WHERE type = $1 AND LOWER(name) = LOWER($2) AND id != $3", [current.type, name.trim(), id]);
            if (duplicate)
                return res.status(400).json({ error: `Another item in ${current.type} already has this name` });
        }
        await schema_1.db.run(`
    ...
    `, [
        ...
        await (0, auth_1.logAuditAction)(req.user?.id || null, "UPDATE", "system_lookups", Number(id), `Updated ${current.type}: ${updatedName}`);
...
router.delete("/lookups/:id", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin"), async (req, res) => {
    try {
        const id = req.params.id;
        const current = await schema_1.db.get("SELECT * FROM system_lookups WHERE id = $1", [id]);
        if (!current)
            return res.status(404).json({ error: "Lookup not found" });
        await schema_1.db.run("DELETE FROM system_lookups WHERE id = $1", [id]);
        await (0, auth_1.logAuditAction)(req.user?.id || null, "DELETE", "system_lookups", Number(id), `Deleted ${current.type}: ${current.name}`);
...
router.post("/lookups/reset", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin"), async (req, res) => {
    try {
        for (const item of DEFAULT_LOOKUPS) {
            await schema_1.db.run(`
        INSERT INTO system_lookups (type, name, description, color, sort_order, is_active)
      ...
      `, [item.type, item.name, item.description, item.color, item.sort_order]);
        ...
        await (0, auth_1.logAuditAction)(req.user?.id || null, "UPDATE", "system_lookups", null, "Reset system lookups to defaults");
...
router.get("/general", async (req, res) => {
    try {
        let list = await schema_1.db.all("SELECT * FROM system_settings ORDER BY category ASC, key ASC");
        ...
        `, [key, val.value, val.category]);
            ...
            list = await schema_1.db.all("SELECT * FROM system_settings ORDER BY category ASC, key ASC");
        }
        const settingsMap = {};
        (list || []).forEach(item => {
...
router.put("/general", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin"), async (req, res) => {
    try {
        ...
        await (0, auth_1.logAuditAction)(req.user?.id || null, "UPDATE", "system_settings", null, "Updated system general settings");
...
router.post("/general/reset", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin"), async (req, res) => {
    try {
        for (const [key, val] of Object.entries(DEFAULT_SETTINGS)) {
            await schema_1.db.run(`
        INSERT INTO system_settings (key, value, category, updated_at)
      ...
      `, [key, val.value, val.category]);
        ...
        await (0, auth_1.logAuditAction)(req.user?.id || null, "UPDATE", "system_settings", null, "Reset general settings to defaults");
...
router.get("/system-info", async (_req, res) => {
    try {
        const [usersCount, membersCount, eventsCount, attendanceCount, ministriesCount] = await Promise.all([
            schema_1.db.get("SELECT COUNT(*) as count FROM users"),
            schema_1.db.get("SELECT COUNT(*) as count FROM members"),
            schema_1.db.get("SELECT COUNT(*) as count FROM events"),
            schema_1.db.get("SELECT COUNT(*) as count FROM attendance"),
            schema_1.db.get("SELECT COUNT(*) as count FROM ministries")
        ]);
        res.json({
            database: "PostgreSQL",
            status: "connected",
