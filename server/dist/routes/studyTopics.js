...
router.get("/", async (req, res) => {
    ...
    `;
        ...
        const topics = await schema_1.db.all(query, params);
        // Summary counts
        const allTopics = await schema_1.db.all("SELECT * FROM bible_study_topics");
        const counts = {
...
router.post("/", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin", "Coordinator"), async (req, res) => {
    try {
        const { title, type = "book", testament_or_category, total_chapters = 1, completed_chapters = 0, status = "in_progress", completed_date, assigned_group_id, assigned_ministry_id, lead_teacher, key_verse, summary_notes } = req.body;
        if (!title) {
    ...
    `, [
        ...
        await (0, auth_1.logAuditAction)(req.user?.id || null, "CREATE", "bible_study_topics", newId, `Created study topic: ${title}`);
...
router.put("/:id", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin", "Coordinator"), async (req, res) => {
    try {
        ...
        const { title, type, testament_or_category, total_chapters, completed_chapters, status, completed_date, assigned_group_id, assigned_ministry_id, lead_teacher, key_verse, summary_notes } = req.body;
    ...
    `, [
        ...
        await (0, auth_1.logAuditAction)(req.user?.id || null, "UPDATE", "bible_study_topics", Number(id), `Updated study topic #${id}`);
...
router.post("/:id/toggle-completed", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin", "Coordinator"), async (req, res) => {
    try {
        const id = req.params.id;
        const current = await schema_1.db.get("SELECT * FROM bible_study_topics WHERE id = $1", [id]);
        if (!current)
    ...
    `, [nextStatus, nextDate, nextChapters, id]);
        await (0, auth_1.logAuditAction)(req.user?.id || null, "UPDATE", "bible_study_topics", Number(id), `Marked topic '${current.title}' as ${nextStatus}`);
        res.json({
...
router.delete("/:id", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin", "Coordinator"), async (req, res) => {
    try {
        const id = req.params.id;
        await schema_1.db.run("DELETE FROM bible_study_topics WHERE id = $1", [id]);
        await (0, auth_1.logAuditAction)(req.user?.id || null, "DELETE", "bible_study_topics", Number(id), `Deleted study topic #${id}`);
