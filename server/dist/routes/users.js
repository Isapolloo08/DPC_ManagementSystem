...
router.get("/roles", async (req, res) => {
    try {
        const roles = await schema_1.db.all(`
...
router.get("/users", auth_1.authMiddleware, async (req, res) => {
    ...
    `;
        ...
        const users = await schema_1.db.all(query, params);
...
router.get("/users/:id", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin"), async (req, res) => {
    try {
...
router.post("/users", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin"), async (req, res) => {
    try {
    ...
    `, [name.trim(), cleanUsername || null, email.trim().toLowerCase(), passwordHash, Number(role_id)]);
        ...
        if (Array.isArray(ministry_ids)) {
            for (const mId of ministry_ids) {
                if (mId) {
                    await schema_1.db.run("INSERT INTO user_ministries (user_id, ministry_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", [newUserId, Number(mId)]);
                }
            }
        }
        // Link member profile if provided
        if (member_id) {
            await schema_1.db.run("UPDATE members SET user_id = $1 WHERE id = $2", [newUserId, Number(member_id)]);
        }
        const role = await schema_1.db.get("SELECT name FROM roles WHERE id = $1", [role_id]);
        await (0, auth_1.logAuditAction)(req.user?.id || null, "CREATE", "users", newUserId, `Created user account '${name.trim()}' (${cleanUsername}) with role ${role?.name || role_id}`);
...
router.put("/users/:id", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin"), async (req, res) => {
    try {
        const id = req.params.id;
        const { name, username, email, password, role_id, ministry_ids, member_id } = req.body;
        const current = await schema_1.db.get("SELECT * FROM users WHERE id = $1", [id]);
        if (!current)
            return res.status(404).json({ error: "User not found" });
        if (email && email.trim().toLowerCase() !== current.email.toLowerCase()) {
            const duplicate = await schema_1.db.get("SELECT id FROM users WHERE LOWER(email) = LOWER($1) AND id != $2", [email.trim(), id]);
            if (duplicate)
        ...
        if (username !== undefined) {
            ...
            if (cleanUsername && cleanUsername !== current.username) {
                const duplicateUsername = await schema_1.db.get("SELECT id FROM users WHERE LOWER(username) = LOWER($1) AND id != $2", [cleanUsername, id]);
        ...
        if (password && password.trim().length >= 6) {
            newHash = bcryptjs_1.default.hashSync(password.trim(), 10);
        }
    ...
    `, [
        ...
        if (Array.isArray(ministry_ids)) {
            await schema_1.db.run("DELETE FROM user_ministries WHERE user_id = $1", [id]);
            for (const mId of ministry_ids) {
                if (mId) {
                    await schema_1.db.run("INSERT INTO user_ministries (user_id, ministry_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", [Number(id), Number(mId)]);
                }
            }
        }
        // Update linked member
        if (member_id !== undefined) {
            await schema_1.db.run("UPDATE members SET user_id = NULL WHERE user_id = $1", [id]);
            if (member_id) {
                await schema_1.db.run("UPDATE members SET user_id = $1 WHERE id = $2", [Number(id), Number(member_id)]);
            }
        }
        await (0, auth_1.logAuditAction)(req.user?.id || null, "UPDATE", "users", Number(id), `Updated user account '${name || current.name}'`);
...
router.delete("/users/:id", auth_1.authMiddleware, (0, auth_1.requireRoles)("Admin"), async (req, res) => {
    try {
        const id = req.params.id;
        if (Number(id) === 1) {
        ...
        await schema_1.db.run("UPDATE members SET user_id = NULL WHERE user_id = $1", [id]);
        await schema_1.db.run("DELETE FROM user_ministries WHERE user_id = $1", [id]);
        await schema_1.db.run("DELETE FROM users WHERE id = $1", [id]);
        await (0, auth_1.logAuditAction)(req.user?.id || null, "DELETE", "users", Number(id), `Deleted user account: ${current.name} (${current.email})`);
