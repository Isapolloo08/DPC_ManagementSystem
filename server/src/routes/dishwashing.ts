 ...
 */
router.post("/generate-cycle", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { 
      ...
      teams_per_turn = 1 // 1 or 2 teams teamed up
    } = req.body;

    const weeks = Math.max(1, Math.min(Number(weeks_count) || 8, 52));
    const isJoint = Number(teams_per_turn) === 2;

    let items: { id: number; name: string; leader_name?: string; ministry_id?: number }[] = [];

    if (cycle_mode === "biblestudy_group") {
      items = await db.all("SELECT id, name, leader_name, ministry_id FROM bible_study_groups ORDER BY id ASC");
    } else {
      const mins = await db.all(`
        SELECT 
          m.id, 
          m.name, 
          (
    ...
    if (replace_existing) {
      ...
      await db.run("DELETE FROM dishwashing_roster WHERE duty_date >= $1 AND status = 'scheduled'", [start_date]);
    }
    ...
    if (req.user) {
      await logAuditAction(
        req.user.id,
        "GENERATE_DISHWASHING_CYCLE",
        "dishwashing_roster",
        0,
        `Generated ${weeks} weeks dishwashing cycle mode: ${cycle_mode} (${isJoint ? 'Joint 2 Teams' : 'Single Team'}) starting ${start_date}`
      );
    }
...
router.post("/", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (req.user) {
      await logAuditAction(req.user.id, "CREATE_DISHWASHING_DUTY", "dishwashing_roster", result.lastInsertRowid, `Created duty for ${assigned_name} ${partner_assigned_name ? `& ${partner_assigned_name}` : ''} on ${duty_date}`);
    }
...
router.put("/:id", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existing = await db.get("SELECT * FROM dishwashing_roster WHERE id = $1", [id]);
    if (!existing) {
    ...
    if (req.user) {
      await logAuditAction(req.user.id, "UPDATE_DISHWASHING_DUTY", "dishwashing_roster", Number(id), `Updated duty #${id} status: ${status || existing.status}`);
    }
 ...
 */
router.post("/:id/swap", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const dutyA = await db.get("SELECT * FROM dishwashing_roster WHERE id = $1", [id]);
    const dutyB = await db.get("SELECT * FROM dishwashing_roster WHERE id = $1", [target_duty_id]);
    ...
    if (req.user) {
      await logAuditAction(req.user.id, "SWAP_DISHWASHING_DUTY", "dishwashing_roster", Number(id), `Swapped duty #${dutyA.id} (${dutyA.assigned_name}) with #${dutyB.id} (${dutyB.assigned_name})`);
    }
...
router.delete("/:id", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    await db.run("DELETE FROM dishwashing_roster WHERE id = $1", [id]);

    if (req.user) {
      await logAuditAction(req.user.id, "DELETE_DISHWASHING_DUTY", "dishwashing_roster", Number(id), `Deleted duty #${id}`);
    }
