...
// Update fund
router.put("/funds/:id", authMiddleware, requireRoles("Admin"), async (req: AuthRequest, res: Response) => {
  try {
...
// Delete fund
router.delete("/funds/:id", authMiddleware, requireRoles("Admin"), async (req: AuthRequest, res: Response) => {
  try {
    const id = req.params.id;
...
router.get("/statement/:memberId", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
